// Controlador de personas. Traduce requests HTTP a llamadas del servicio.
import type { Request, Response } from "express";
import { PersonaService } from "../services/personaService";
import { idParamSchema, paginationQuerySchema } from "../types/validators";

export class PersonaController {
  constructor(private readonly service = new PersonaService()) {}
  ...
  expedientes = async (request: Request, response: Response) => {
    const { id } = idParamSchema.parse(request.params);
    const data = await this.service.listExpedientes(id);
    response.json({ data });
  };
